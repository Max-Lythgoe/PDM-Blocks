...
/**
 * Helpers for the PDM Tabs Block.
 */
 ...
 * @return array
 */
if (! function_exists('pdm_tabs_extract_tab_data')) {
    function pdm_tabs_extract_tab_data(array $inner_blocks): array
    {
 ...
 */
if (! function_exists('pdm_tabs_resolve_spacing_size')) {
    function pdm_tabs_resolve_spacing_size($value, $defaultValue = '0px')
    {
        if (is_string($value)) {
 ...
 */
if (! function_exists('pdm_tabs_get_gap_styles')) {
    function pdm_tabs_get_gap_styles($attributes)
    {
        ...
        if (is_string($blockGap)) {
            $main = $cross = pdm_tabs_get_gap_css_value($blockGap);
        } else {
            $main = pdm_tabs_get_gap_css_value($blockGap['top'] ?? '0.5em');
            $cross = pdm_tabs_get_gap_css_value($blockGap['left'] ?? '0.5em');
        }
 ...
 */
if (! function_exists('pdm_tabs_get_gap_css_value')) {
    function pdm_tabs_get_gap_css_value($value)
    {
        if (preg_match('/^var:preset\|spacing\|[\w-]+$/', $value)) {
 ...
 */
if (! function_exists('pdm_tabs_resolve_color_value')) {
    function pdm_tabs_resolve_color_value($colorValue, $fallback = '')
    {
 ...
 */
if (! function_exists('pdm_tabs_get_typography_classes')) {
    function pdm_tabs_get_typography_classes($attributes)
    {
        $typography_classes = [];
 ...
 */
if (! function_exists('pdm_tabs_get_typography_styles')) {
    function pdm_tabs_get_typography_styles($attributes)
    {
        $styles = [];
 ...
 */
if (! function_exists('pdm_tabs_get_border_color_classes')) {
    function pdm_tabs_get_border_color_classes($attributes)
    {
 ...
 */
if (! function_exists('pdm_tabs_get_tabs_border_styles')) {
    function pdm_tabs_get_tabs_border_styles(array $attributes): string
    {
        $styles = [];
        ...
        // Fallback: emit currentColor for any side that has width/style but no explicit color
        $shorthandWidth = _wp_array_get($attributes, ['style', 'border', 'width'], '');
        $shorthandStyle = _wp_array_get($attributes, ['style', 'border', 'style'], '');
        $shorthandColor = _wp_array_get($attributes, ['style', 'border', 'color'], '');
        if (($shorthandWidth || $shorthandStyle) && ! $shorthandColor) {
            // Only add shorthand fallback if no per-side colors override it
            $anyPerSideColor = false;
            foreach (['top', 'right', 'bottom', 'left'] as $side) {
                if (_wp_array_get($attributes, ['style', 'border', $side, 'color'], '')) {
                    $anyPerSideColor = true;
                    break;
                }
            }
            if (! $anyPerSideColor) {
        ...
        foreach (['top', 'right', 'bottom', 'left'] as $side) {
            $sideWidth = _wp_array_get($attributes, ['style', 'border', $side, 'width'], '');
            $sideStyle = _wp_array_get($attributes, ['style', 'border', $side, 'style'], '');
            $sideColor = _wp_array_get($attributes, ['style', 'border', $side, 'color'], '');
            if (($sideWidth || $sideStyle) && ! $sideColor) {
                $styles[] = "border-{$side}-color: currentColor;";
            }
        }
 ...
 */
if (! function_exists('pdm_tabs_generate_styles')) {
    function pdm_tabs_generate_styles($attributes = [])
    {
        $styles = pdm_tabs_get_gap_styles($attributes);

        // Padding
        $padding = $attributes['tabPadding'] ?? [];
        $styles['--bbb-tab-padding'] = sprintf(
            '%s %s %s %s',
            pdm_tabs_resolve_spacing_size($padding['top'] ?? null, '5px'),
            pdm_tabs_resolve_spacing_size($padding['right'] ?? null, '15px'),
            pdm_tabs_resolve_spacing_size($padding['bottom'] ?? null, '5px'),
            pdm_tabs_resolve_spacing_size($padding['left'] ?? null, '15px')
        );
        ...
        foreach ($colorDefaults as $state => $defaults) {
            ...
            $styles["--bbb-tab-text-{$state}-color"] = pdm_tabs_resolve_color_value(
                $textColor,
                $defaults['text']
            );
            ...
            $styles["--bbb-tab-background-{$state}-color"] = pdm_tabs_resolve_color_value(
                $backgroundColor,
                $defaults['bg']
            );
            ...
            $styles["--bbb-tab-icon-{$state}-color"] = pdm_tabs_resolve_color_value(
                $iconColor,
                $defaults['icon']
            );
        }
 ...
 */
if (! function_exists('pdm_tabs_get_tabs_container_styles')) {
    function pdm_tabs_get_tabs_container_styles(array $attributes): string
    {
        $styles = [];

        // Border styles
        $styles[] = pdm_tabs_get_tabs_border_styles($attributes);
        ...
        $styles[] = sprintf(
            'padding: %s %s %s %s;',
            pdm_tabs_resolve_spacing_size($padding['top'] ?? null, '0px'),
            pdm_tabs_resolve_spacing_size($padding['right'] ?? null, '0px'),
            pdm_tabs_resolve_spacing_size($padding['bottom'] ?? null, '0px'),
            pdm_tabs_resolve_spacing_size($padding['left'] ?? null, '0px')
        );
 ...
 */
if (! function_exists('pdm_tabs_get_color_classes')) {
    function pdm_tabs_get_color_classes($attributes)
    {
        $classes = [];
 ...
 * @return string A string of CSS declarations.
 */
if (! function_exists('pdm_tabs_get_tab_button_border_styles')) {
    function pdm_tabs_get_tab_button_border_styles(array $attributes): string
    {
        $styles = [];

        // Tab Border styles
        $styles[] = pdm_tabs_get_tabs_border_styles(['style' => $attributes['tabBorder'] ?? []]);
