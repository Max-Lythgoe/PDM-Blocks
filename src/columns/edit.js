...
export default function Edit({attributes, setAttributes}) {
    const ALLOWED_BLOCKS = [ 'pdm/column' ];

    const TEMPLATE = Array.from({ length: attributes.desktopColumns }, () => ['pdm/column', {}]);
    ...
    const blockProps = useBlockProps({
        className: attributes.verticalAlignment ? `is-vertically-aligned-${attributes.verticalAlignment}` : '',
        style: style
    });
    ...
    return (
        <>
            ...
            <div {...blockProps}>
                <InnerBlocks template={TEMPLATE} allowedBlocks={ALLOWED_BLOCKS} templateLock={false} renderAppender={InnerBlocks.ButtonBlockAppender} orientation="horizontal" />
            </div>
        </>
    );
}