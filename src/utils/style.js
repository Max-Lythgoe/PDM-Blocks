 ...
 */
const fillMissingBorderColors = ( style ) => {
	const sides = [ 'Top', 'Right', 'Bottom', 'Left' ];
	const filled = { ...style };

	// Shorthand
	if ( ( filled.borderWidth || filled.borderStyle ) && ! filled.borderColor ) {
		filled.borderColor = 'currentColor';
	}

	// Per-side
	for ( const side of sides ) {
		if (
			( filled[ `border${ side }Width` ] ||
				filled[ `border${ side }Style` ] ) &&
			! filled[ `border${ side }Color` ]
		) {
			filled[ `border${ side }Color` ] = 'currentColor';
		}
	}
...
export function getTabButtonStyles( attributes, isActive ) {
	...
	return {
		style: fillMissingBorderColors( borderProps.style ),
	};
}
