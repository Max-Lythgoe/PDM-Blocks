 ...
 */
const { Fill, Slot } = createSlotFill(Symbol('PdmTabsList'));

/**
 * TabFill Component
 *
 * This component registers a Fill for the PdmTabsList Slot.
 *
 * @param {Object}          props
...
export const TabFill = ({ children, tabsClientId }) => {
	return <Fill name={`PdmTabsList-${tabsClientId}`}>{children}</Fill>;
};

/**
 * PdmTabsListSlot Component
 *
...
export const TabsListSlot = ({ tabsClientId, attributes }) => {
	...
	return (
		<Slot
			name={`PdmTabsList-${tabsClientId}`}
			bubblesVirtually
			as="div"
			role="tablist"
			className={clsx(className, 'pdm-tabs-buttons')}
			style={style}
		/>
	);
};
