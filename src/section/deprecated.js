...
// v3: content-wrapper applies layout (useInnerBlocksProps); background is NOT yet
// aria-hidden. Reproduces the currently-saved markup so existing sections migrate.
const v3 = {
	attributes: {
		...
		layout: { type: 'object', default: { type: 'constrained' } },
	},
	save( { attributes } ) {
		const {
			imageURL,
		...
		const innerBlocksProps = useInnerBlocksProps.save( { className: 'content-wrapper' } );
